...
func SetGetOwner(context tele.Context) error {
	...
	if len(context.Args()) != 1 || context.Message().ReplyTo == nil {
		return utils.SendAndRemove("Пример использования: <code>/setgetowner {гет}</code> в ответ пользователю, которого нужно задать владельцем.", context)
	}
	...
	if result.RowsAffected != 0 {
		...
		return utils.SendAndRemove(fmt.Sprintf("Владелец гета <code>%v</code> изменён на %v.", get.Name, utils.MentionUser(context.Message().ReplyTo.Sender)), context)
	} else {
