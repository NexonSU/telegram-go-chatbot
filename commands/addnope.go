...
func AddNope(context tele.Context) error {
	...
	if (context.Message().ReplyTo == nil && len(context.Args()) == 0) || (context.Message().ReplyTo != nil && len(context.Args()) != 0) {
		return utils.ReplyAndRemove("Пример использования: <code>/addnope {текст}</code>\nИли отправь в ответ на сообщение с текстом <code>/addnope</code>", context)
	}
	...
	if result.Error != nil {
		return utils.ReplyAndRemove(fmt.Sprintf("Не удалось добавить nope, ошибка:\n<code>%v</code>", result.Error.Error()), context)
	}
	return utils.ReplyAndRemove(fmt.Sprintf("Nope добавлен как <code>%v</code>.", nope.Text), context)
}
